...
const changeUserRole = async (uid) => {
    ...
    if (!user) throw customErrors.notFoundError("User not found"); //validamos que este el usuario

    if (user.role === "user" && user.documents.length < 3) throw customErrors.badRequestError("You need to show all the documentation that is required"); //Si son menos de 3 documentos indica error
...
const addDocuments = async (uid, reqFiles) => {
    ...
    const userDocuments = files.map((file) => { //armo un objeto formateado como en el modelo
        return {
            name: file.filename,
            reference: file.path,
        };
