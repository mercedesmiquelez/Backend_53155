...
router.get("/sms", async (req, res) => { //hacemos este endpoint para enviar mensajes
  await sendSMS("+541131851308", "Coder es los más !!!"); //importamos la funcion
...
router.get("/email", async (req, res) => {
  ...
  const template = `
    ...
    `;//podemos mandar un email personalizado para cada usuario y agregamos al template la imagen

  await sendMail("mercedesmiquelez@gmail.com", "Teste nodemailer", "Este es un mensaje de prueba", template);

  return res.status(200).json({ status: "ok", msg: "Email enviado" });
});

router.get("/usersmocks", async (req, res) => {
  const users = generateUsersMocks(40000); //Le indicamos la cantidad de fake usuarios que queremos que se generen
